...
# 模型2:alexnet
alexnet = models.densenet161(pretrained=False)  # 加载预训练的alexnet模型
# 由于AlexNet的第一个卷积层步长为4，我们可以通过调整卷积核大小来适应新的输入尺寸
# 替换AlexNet的第一个卷积层

alexnet.classifier = nn.Sequential(
    nn.Linear(2208, 4096, bias=True),
    nn.ReLU(),
    nn.Dropout(0.4),
    nn.Linear(4096, 512, bias=True),
    nn.ReLU(),
...
# 模型3:resnet50
resnet101 = models.densenet169(pretrained=False)  # 加载预训练的resnet50模型
resnet101.classifier = nn.Sequential(
    nn.Linear(1664, 4096, bias=True),
    nn.ReLU(),
    nn.Dropout(0.4),
    nn.Linear(4096, 512, bias=True),
    nn.ReLU(),
...
model2_name = 'densenet161'
model3_name = 'densenet169'
model4_name = 'svc'
...
def train_and_valid(model_1, model_2, model_3, model_svc,
    ...
    for epoch in range(epochs):
        ...
        with open(f'./record/log.txt', 'a', encoding='utf8') as f:
            f.write(f"{epoch + 1}\n\t"
...
model1_name = 'model_1_desnet'
model2_name = 'model_2_desnet'
model3_name = 'model_3_desnet'
model4_name = 'model_4_demo'
model5_name = 'model_ensemble_desnet'
