...
train_dataset, test_dataset, train_labels, test_labels = train_test_split(
    ...
    test_size=0.3,
    random_state=42,
    stratify=full_labels  # 根据full_labels的值进行分层划分
)

# ===================打印训练集中的前两个样本====================
# print("第一个样本：")
# print("图像：", train_dataset[0][0])
# print("多光谱数据：", train_dataset[0][1])
# print("类别标签：", train_dataset[0][2])

# print("第二个样本：")
# print("图像：", train_dataset[1][0])
# print("多光谱数据：", train_dataset[1][1])
# print("类别标签：", train_dataset[1][2])
...
print("-------数据划分完成 ~~~ -------")
model_svc = RandomForestClassifier(random_state=42)
# model_svc = RandomForestClassifier(random_state=42)
# model_svc = DecisionTreeClassifier(random_state=42)
...
for images, X_batch, y_batch in train_dataloader:
    ...
    train_loss += log_loss(y_batch, model_svc.predict_proba(X_batch), labels=model_svc.classes_)
...
for images, X_batch, y_batch in test_dataloader:
    ...
    y_batch = y_batch.numpy()
    predictions = model_svc.predict(X_batch)
    test_loss += log_loss(y_batch, model_svc.predict_proba(X_batch), labels=model_svc.classes_)
...
# 模型1:densenet121
densenet121 = models.resnet50(pretrained=False,num_classes=3)
# 模型2:alexnet
alexnet = models.resnet152(pretrained=False, num_classes=3)  # 加载预训练的alexnet模型
# 由于AlexNet的第一个卷积层步长为4，我们可以通过调整卷积核大小来适应新的输入尺寸
# 模型3:resnet50
resnet101 = models.resnet34(pretrained=False, num_classes=3)  # 加载预训练的resnet50模型
# 将模型移动到GPU上
...
model2_name = 'resnet152'
model3_name = 'resnet34'
model4_name = 'svc'
model5_name = 'ensemble_resnet'
...
def train_and_valid(model_1, model_2, model_3, model_svc,
    ...
    for epoch in range(epochs):
        ...
        # if best_acc_4 < avg_valid_acc_4:
        #     best_acc_4 = avg_valid_acc_4
        #     best_epoch_4 = epoch + 1
...
model1_name = 'model_1_6'
model2_name = 'model_2_6'
model3_name = 'model_3_6'
model4_name = 'model_4_6'
model5_name = 'model_ensemble_6'
